...
async function loadIntegrationStatus() {
  try {
    ...
    const names = { 'Google sign-in': 'google', ChatGPT: 'chatgpt', Claude: 'claude', Gemini: 'gemini', 'File storage': 'storage', Stripe: 'stripe' };
    document.querySelectorAll('.integration-action').forEach((button) => {
      const configured = statuses[names[button.dataset.service]];
      button.textContent = configured ? 'Connected' : 'Set key';
      button.classList.toggle('connected', configured);
      button.dataset.configured = configured;
    });
  } catch {
    showToast('Could not check integration status.');
  }
}
...
const gymDialog = document.querySelector('#gymDialog');
document.querySelector('#closeGym')?.addEventListener('click', () => gymDialog.close());
document.querySelectorAll('.gym-card').forEach((card) => card.addEventListener('click', () => {
  document.querySelector('#gymTitle').textContent = card.dataset.gym;
  document.querySelector('#gymRating').textContent = card.dataset.rating;
  document.querySelector('#gymReviews').textContent = card.dataset.reviews;
  document.querySelector('#gymAddress').textContent = `${card.dataset.address} · ${card.dataset.distance}`;
  document.querySelector('#gymCrowd').textContent = card.dataset.crowd;
  document.querySelector('#googleMapsLink').href = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${card.dataset.gym}, ${card.dataset.address}`)}`;
  gymDialog.showModal();
}));
